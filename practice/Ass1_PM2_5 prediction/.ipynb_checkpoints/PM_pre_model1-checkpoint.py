...
#y = W1X1 + W2X2 +...WnXn + b
#seen 18(features) * 9 (hours) as functions' features
...
def preprocess_train_data(train_set):
    ...
    #9-hrs data   shape : 1, 18 * 9 
    #10th-hr pm2.5   shape: 1, 1
    train_x = np.empty((12 * (480 - 9), 18 * 9), dtype = float)                
    train_y = np.empty((12 * (480 - 9), ), dtype = float)                

    
    #for each continuous 10 hours, use the every 9 columns data in a month as x
...
def training(model_fun, train_x, train_y):
    ...
    for T in range(iteration):
        ...
        for i in range(len(train_x)):
            ...
            #lambda = 0.1
            #t_w_grad, t_b_grad = model_fun(train_x[i], train_y[i], w, b, 0.1)
            #lambda = 0.01
...
def predict():
    ...
    for i in range(num_of_test):
        input_features = test_x[i * 18 : (i + 1) * 18, : ].reshape(1, -1)
